#!/usr/bin/env python3
"""
Graph Attention Networks (GAT) Experiment Runner
...
def print_experiment_header(**config):
    """Print GAT experiment header with configuration."""
    print(f"\n{'='*70}")
    print(f"Graph Attention Networks (GAT) Experiment")
    print(f"Paper: arXiv:1710.10903v3")
    print(f"{'='*70}")
...
def run_experiment(args):
    ...
    experiment_config = {
        ...
        'architecture': f"{args.num_heads}-head attention, {args.hidden_dim}d hidden",
        'layers': f"{args.num_layers} GAT layer(s)",
    ...
    results = train_gat(model_config, training_config, data_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Graph Attention Networks (GAT) Experiment Runner",
...
Examples:
  ...
  python experiment.py --num-heads 4 --hidden-dim 16 --num-layers 2
  
  # Training configuration
  python experiment.py --total-steps 1000 --eval-interval 25 --patience 5
  
  # Regularization study
  python experiment.py --dropout 0.5 --no-skip-connections
  
GAT Architecture:
  - Multi-head attention mechanism for node aggregation
  - Skip connections for better gradient flow  
  - Dropout regularization for generalization
  - Applied to CORA citation network classification
  - Step-based training with validation-based early stopping
    ...
    parser.add_argument('--hidden-dim', type=int, default=8,
                        help='Hidden dimension per attention head (default: 8)')
